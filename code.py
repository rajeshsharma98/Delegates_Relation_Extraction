...
def get_jobs():
    ...
    with open("job_lists.txt", "w") as output:
        output.write(str(links))
    
    print('File stored at: ','job_lists.txt')
    return links
...
def step1(file_name):
    '''
    Collect website links - scraping google search results
    '''
    ...
    df.to_csv('data_fetchedLinks.csv')
    return str('data_fetchedLinks.csv')

def step2(filename2):
    '''
    Collect websites textual data 
    '''
    ...
    for i in trange(df.shape[0]):
        ...
        with open('fetched/'+filename+' '+str(i)+".json", "w") as outfile:
            outfile.write(json_object)

        df1.loc[i,'FileName'] = filename
    df1.to_csv('metadata_jsons.csv')
    return str(('metadata_jsons.csv'))

def step3(filename3):
    ''' 
    named Entity Recognition and store all the fetched Person Names, Job Names and Organization Names as extra columns in metadata (or metadata_jsons.csv)  
    '''
    ...
    for file in tqdm(files):
        ...
        if file != '.DS_Store':
            ...
            dd = pd.read_csv('jobLists_subset.csv')
